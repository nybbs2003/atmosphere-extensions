...
import org.atmosphere.socketio.transport.SocketIOPacketBinary;
...
public class SocketIOAtmosphereInterceptor implements AtmosphereInterceptor {
	...
	/**
	 * See <a href="https://github.com/LearnBoost/socket.io/wiki/Configuring-Socket.IO#wiki-server">https://github.com/LearnBoost/socket.io/wiki/Configuring-Socket.IO#wiki-server</a><br>
	 * The timeout for the client, we should receive a heartbeat from the server within this interval. This should be greater than the heartbeat interval. <b>This value is sent to the client after a
	 * successful handshake.</b><br>
	 * In milliseconds.
	 */
				...
				if (!SocketIOAtmosphereHandler.class.isAssignableFrom(atmosphereHandler.getClass())) {
					response.asyncIOWriter(new AsyncIOWriterAdapter() {
						...
						public AsyncIOWriter write(AtmosphereResponse r, byte[] data) throws IOException {
							SocketIOSessionOutbound outbound = (SocketIOSessionOutbound) request.getAttribute(SocketIOAtmosphereHandler.SOCKETIO_SESSION_OUTBOUND);
							if (outbound != null) {
								outbound.sendMessage(new SocketIOPacketBinary(data));
							} else {
								SocketIOSessionManagerImpl.SocketIOProtocol p = (SocketIOSessionManagerImpl.SocketIOProtocol) r.request().getAttribute(SOCKETIO_PACKET);
								if (p == null) {
